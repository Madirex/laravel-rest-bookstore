            ...
            @foreach ($shops as $shop)
                ...
                <div class="shop-card">
                    ...
                    <div class="shop-actions">
                        <a class="btn btn-primary btn-sm" href="{{ route('shops.show', $shop->id) }}"><i
                                class="fas fa-info-circle"></i></a>
                        @if(auth()->check() && auth()->user()->hasRole('admin'))
                            <a class="btn btn-secondary btn-sm" href="{{ route('shops.edit', $shop->id) }}"><i
                                    class="fas fa-edit"></i></a>
                            <a class="btn btn-danger btn-sm delete-btn" data-toggle="modal"
                               data-target="#confirmDeleteModal{{ $shop->id }}"><i class="fas fa-trash-alt"></i></a>
                        @endif
                    </div>
                </div>

                <!-- Modal de Confirmación de eliminación -->
                <div class="modal fade" id="confirmDeleteModal{{ $shop->id }}" tabindex="-1" role="dialog"
